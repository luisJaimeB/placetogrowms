...
const selectPlan = (plan) => {
    props.form.amount = plan.amount;
    props.form.plan = [plan];
        ...
        <div class="max-w-2xl w-full bg-white rounded-xl shadow-2xl overflow-hidden transform transition duration-500">
            ...
            <form @submit.prevent="$emit('submit')" class="p-6 space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <!-- Tarjetas -->
                    <div class="mt-16 container space-y-12 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-x-8">
                        <div
                            v-for="plan in plans"
                            :key="plan.id"
                            @click="selectPlan(plan)"
                            :class="{
                                'relative p-8 border rounded-2xl shadow-sm flex flex-col cursor-pointer': true,
                                'border-gray-200': form.amount !== plan.amount,
                                'border-indigo-500 bg-gray-100': form.amount === plan.amount
                            }"
                        >
                            <div class="flex-1">
                                <h3 class="text-xl font-semibold ">{{ plan.name }}</h3>
                                <p class="mt-4 flex items-baseline ">
                                    <span class="text-3xl font-extrabold tracking-tight">${{ plan.amount }}</span>
                                    <span class="ml-1 text-xl font-semibold">/{{plan.periodicity}}</span>
                                </p>
                                <p class="mt-6">El cobro se realizará {{ plan.interval }} vez, con periodicidad {{ plan.periodicity }}</p>
