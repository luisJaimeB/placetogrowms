                ...
                <div class="mt-4 shadow-lg rounded-lg overflow-hidden mx-4 md:mx-10">
                    <table class="w-full table-fixed bg-white border rounded-lg">
                        ...
                        <tr class="bg-gray-100">
                            <th class="w-1/4 py-4 px-6 text-center text-gray-600 font-bold uppercase">{{ t('fields.id') }}</th>
                            <th class="w-1/4 py-4 px-6 text-center text-gray-600 font-bold uppercase">{{ t('fields.name') }}</th>
                            <th class="w-1/4 py-4 px-6 text-center text-gray-600 font-bold uppercase">{{ t('fields.email') }}</th>
                            <th class="w-1/4 py-4 px-6 text-center text-gray-600 font-bold uppercase">{{ t('actionsLabel') }}</th>
                        </tr>
                        </thead>
                        <tbody class="bg-white">
                        <tr v-for="user in users.data" :key="user.id" class="transition-transform duration-300 ease-in-out transform hover:scale-95 cursor-pointer">
                            <td class="py-4 px-6 border-b border-gray-200 text-center">{{ user.id }}</td>
                            <td class="py-4 px-6 border-b border-gray-200 text-center">{{ user.name }}</td>
                            <td class="py-4 px-6 border-b border-gray-200 text-center">{{ user.email }}</td>
                            <td class="py-4 px-6 border-b border-gray-200 text-center">
                                <button class="bg-green-400 text-white px-2 py-1 rounded mr-2" v-if="$page.props.user.permissions.includes('users.update')">
                                ...
                                <button @click="deleteUser(user.id)" class="bg-red-400 text-white px-2 py-1 rounded" v-if="$page.props.user.permissions.includes('users.delete')">
